...
void main() {
    ...
    for (int i = 0; i < NumRaysPerPixel; i++) {
        totalLight += Trace(ray, NumMaxBounces, uv * pushConstants.uTime * (i+1));
    }

    uint pixelIndex = uint(floor(gl_FragCoord.x + gl_FragCoord.y * pushConstants.uResolution.x));

    vec4 currentColor = vec4(totalLight / NumRaysPerPixel, 1.0);
    vec4 previousColor = pixels.data[pixelIndex];
    fragColor = mix(previousColor, currentColor, 0.1);

    pixels.data[pixelIndex] += currentColor;
}